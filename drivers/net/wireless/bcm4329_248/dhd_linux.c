...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (system_rev == 0 || system_rev == 1) {
		if (strstr(firmware_path, "fw_bcm4329.bin")) {
			strcpy(firmware_path, "/system/etc/firmware/fw_bcm4329.bin.bg");
		}
		else {
			strcpy(firmware_path, "/system/etc/firmware/fw_bcm4329_apsta.bin.bg");
		}
		strcpy(nvram_path, "/system/etc/calibration.bg");
	}
#endif
