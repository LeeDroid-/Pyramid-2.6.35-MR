...
static void check_vbus_in(struct work_struct *w)
{
	...
	if (vbus != vbus_in) {
		...
		msm_hsusb_set_vbus_state(vbus_in);
...
#ifdef CONFIG_CABLE_DETECT_ACCESSORY_BY_ADC
static int cable_detect_get_adc(void)
{
	struct cable_detect_info *pInfo = &the_cable_info;
	int mean_adc;
...
static void dock_detect_init(struct cable_detect_info *pInfo)
{
	...
	ret = set_irq_wake(pInfo->dockpin_irq, 1);
	if (ret < 0) {
		CABLE_ERR("[GPIO DOCK] %s: set_irq_wake failed\n", __func__);
		goto err;
	}
